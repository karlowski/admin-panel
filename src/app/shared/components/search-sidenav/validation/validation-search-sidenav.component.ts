...
import { loadValidationQuery } from 'src/app/main/validations/store/validations.actions';

@Component({
  selector: 'app-validation-search-sidenav',
  templateUrl: './validation-search-sidenav.component.html',
  styleUrls: ['./validation-search-sidenav.component.scss']
})
export class ValidationSearchSidenavComponent extends Unsubscriber implements OnInit {
  ...
  constructor(private store: Store<IWorkflowsNgRxState>,
    ...
    this.urlFilterService.filterSubject.pipe(
      ...
      const { state, status, dummy, createdStart, createdEnd } = filters;

      this.form.controls.state.setValue(state);
      this.form.controls.status.setValue(status);
      this.form.controls.dummy.setValue(dummy);

      if (createdStart) {
        this.form.controls.createdStart.setValue(new Date(+createdStart));
      }

      if (createdEnd) {
        this.form.controls.createdEnd.setValue(new Date(+createdEnd));
      }
    });
  }
